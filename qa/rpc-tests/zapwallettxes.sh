...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars KBCD and KBCCLI may be used to specify the exact binaries used"
        exit 1
fi
...
$KBCD -zapwallettxes=1 $B1ARGS &
B1PID=$!
$KBCD -zapwallettxes=2 $B2ARGS &
B2PID=$!
