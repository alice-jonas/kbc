...
*Setup instructions for a Gitian build of Kbc using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Kbc
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-kbc-host/README.md).
...
Clone the git repositories for kbc and Gitian and then checkout the kbc version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/kbc/kbc
cd kbc
...
Follow the instructions in [doc/release-process.md](release-process.md) in the kbc repository
...
To build Kbc (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the kbc repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/kbc/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/kbc/kbc
...
For example:
```bash
URL=https://github.com/laanwj/kbc.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit kbc=${COMMIT} --url kbc=${URL} ../kbc/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit kbc=${COMMIT} --url kbc=${URL} ../kbc/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit kbc=${COMMIT} --url kbc=${URL} ../kbc/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/kbc-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/kbc-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/kbc-build.assert
```
...
[kbc/gitian.sigs](https://github.com/kbc/gitian.sigs/) repository, or if that's not possible create a pull
